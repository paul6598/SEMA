...
class LLM_Intention(Transform):
    def __init__(self, algorithm: Larlpassage, group: str):
        super().__init__(
    ...
    def get_llm_intention(self, group: str, tensordict: TensorDictBase = None): # -> [batch(120), agent(2)]
        ...
        with torch.no_grad():
            contents = self.algorithm.pipe(state_description, max_new_tokens=300, batch_size= len(state_description))#len(state_description)) # , max_length = None, max_new_tokens=100
        # print("*" * 50)
...
@dataclass
class LarlpassageConfig(AlgorithmConfig):
