...
class Sema(Algorithm):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.training_rate = 0.0  # 초기 training rate 설정
        # self.pipe = pipeline(
        #     "text-generation",
    ...
    def entropy_decay(self, group: str, losses: dict, training_rate: float):
        ...
        if training_rate >= 0.1 and self.save_mlp:
            self.save_mlp_weights(progress=f"{int(training_rate.item() * 100)}%")

        self.training_rate = training_rate.item() if isinstance(training_rate, torch.Tensor) else training_rate
    
    def save_mlp_weights(self, progress: str = "latest", task: str = "giveway", seed: int = 1):
...
class LLM_Intention(Transform):
            ...
            base_prompt = [
            {
                ...
                "content": [{
                    ...
                    "text": (
                        ...
                        "- Refuge: Located at x=0, close to Agent 1. deep in y direction (y > 0.4).\n"
                        ...
                        # "YIELDING RULE: The corridor is narrow, making it difficult for agents to pass each other simultaneously. To prevent collisions, strategic yielding is required when agents are in close proximity.\n"
                    )
                }]
            },
            {
    ...
    def _step(self, tensordict: TensorDictBase, next_tensordict: TensorDictBase) -> TensorDictBase:
        ...
        if LLM_mask.any() or prev_raw_vector is None:
            ...
            if getattr(self.algorithm, "training_rate", 0.0) >= 0.0: # intention이 유효한 경우에만 로깅
                with torch.no_grad():
