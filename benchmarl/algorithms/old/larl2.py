...
class LarlPPOLossWithAlignment(ClipPPOLoss):
    def __init__(self, algorithm, group, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.algorithm = algorithm
        self.group = group
        self.alpha = 5.0 # 초기 Alpha

    def forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        # 1. 원본 PPO Loss 계산 (내부적으로 tensordict_out 반환)
        loss_td = super().forward(tensordict)
        
        # 2. SBERT 원본 임베딩 가져오기
        sbert_embed = tensordict.get((self.group, "sbert_embedding"))
        # 3. Linear Layer 통과 (미분 가능)
        llm_target = self.algorithm.linear(sbert_embed)
        
        # 4. 에이전트 Action(평균 loc) 가져오기
        # ClipPPOLoss 내부에는 self.actor_network가 존재합니다.
        td_copy = tensordict.clone()
        self.actor_network(td_copy)
        agent_loc = td_copy.get((self.group, "loc"))
        # 5. MSE Alignment Loss 계산 및 저장
        alignment_loss = torch.nn.functional.mse_loss(agent_loc, llm_target)
        loss_td.set("loss_alignment", alignment_loss * self.alpha)
        loss_td.set("alpha", self.alpha) 
        return loss_td
...
class Larl(Algorithm):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.linear = nn.Linear(384, self.intention_vector_size).to(self.device)
        # self.linear = nn.Sequential(
        #     nn.Linear(384, 128),
        #     nn.ReLU(),
        #     nn.Linear(128, self.intention_vector_size),
        #     nn.Tanh()
        # ).to(self.device)
    ...
    def process_env_fun(self, env_fun):
        ...
        if (group_name, "observation") in self.observation_spec.keys(True):
            ...
            if (group_name, "sbert_embedding") not in self.observation_spec.keys(True):
                new_shape = torch.Size([*orig_spec.shape[:-1], orig_spec.shape[-1] + self.intention_vector_size])
                
                self.observation_spec[group_name, "observation"] = type(orig_spec)(
                    shape=new_shape, 
        ...
        # state 업데이트 (Critic 용)
        if hasattr(self, "state_spec") and self.state_spec is not None:
            if (group_name, "state") in self.state_spec.keys(True):
                orig_spec = self.state_spec[group_name, "state"]
                if (group_name, "sbert_embedding") not in self.observation_spec.keys(True):
                    new_shape = torch.Size([*orig_spec.shape[:-1], orig_spec.shape[-1] + self.intention_vector_size])
                    
                    self.state_spec[group_name, "state"] = type(orig_spec)(
                        shape=new_shape, 
                        device=orig_spec.device
                    )
    ...
    def _get_loss(self, group: str, policy_for_loss: TensorDictModule, continuous: bool) -> Tuple[LossModule, bool]:
            ...
            loss_module = LarlPPOLossWithAlignment(
                algorithm=self,
                group=group,
                actor=policy_for_loss,
                critic=self.get_critic(group),
    ...
    def entropy_decay(self, group: str, losses: dict, training_rate: float):
        ...
        if self.LLM_interval_decay:
            self.LLM_interval = int(end_interval - (end_interval - start_interval) * torch.pow(training_rate, 0.5)) # 초기에는 300에서 시작해서 점차 줄어듦
            #self.LLM_interval = int(start_interval + (end_interval - start_interval) * training_rate)

        # init_alpha = 0.1
        # decay_rate = 10.0
        # new_alpha = init_alpha * math.exp(-decay_rate * training_rate)
        # if new_alpha < 0.0001:
        #     new_alpha = 0.0
        # if hasattr(losses[group], "alpha"):
        #     losses[group].alpha = max(0.0, float(new_alpha))
    ...
    def _get_policy_for_loss(
        ...
        actor_input_spec = Composite(
            {
                group: Composite(
                    {"observation": self.observation_spec[group, "observation"].clone().to(self.device)},
                    shape=(n_agents,)  
                )
            }
        )
    ...
    def process_loss_vals(self, group: str, loss_vals: TensorDictBase) -> TensorDictBase:
            ...
            if "loss_alignment" in loss_vals.keys():
                total_obj += loss_vals["loss_alignment"]
                del loss_vals["loss_alignment"]
...
class LLM_Intention_backup(Transform):
    def __init__(self, algorithm: Larl, group: str):
        super().__init__(
            in_keys=[(group, "observation")], 
            out_keys=[(group, "observation"),
        ...
        self.in_keys = [(self.group, "observation")]
        self.out_keys = [(self.group, "observation"),
                         (self.group, "sbert_embedding")]


        self.n_agents = len(self.algorithm.group_map[group]) # 2
        self.cached_raw_vector = None
    ...
    def _step(self, tensordict: TensorDictBase, next_tensordict: TensorDictBase) -> TensorDictBase:
        ...
        LLM_mask = (step_count % self.algorithm.LLM_interval == 0) | (self.cached_vector is None)

        if LLM_mask.any() or self.cached_vector is None:
            # intention = [0 for _ in range(batch_size)]
            intention = self.get_llm_intention(self.group, next_tensordict)
            raw_vector, vector = self.intention_to_vector(intention) # shape: (32, 16)
            # print(f"LLM_intention : {intention[0]}")
            # print(f"vector : {vector[0]}")
            #self.cached_vector = vector.unsqueeze(1).expand(batch_size, self.n_agents, -1).detach() # shape: (batch_size, n_agents, 16)
            if not hasattr(self, "cached_raw_vector") or self.cached_raw_vector is None:
                self.cached_raw_vector = raw_vector.detach()
            
            self.cached_vector = torch.where(
                LLM_mask.unsqueeze(-1).unsqueeze(-1),
                vector.detach(),
                self.cached_vector
            )
            self.cached_raw_vector = torch.where(
                LLM_mask.unsqueeze(-1).unsqueeze(-1),
                raw_vector.detach(),
                self.cached_raw_vector
            )
        new_obs = torch.cat([curr_obs, self.cached_vector], dim=-1)
        next_tensordict.set((self.group, "observation"), new_obs)
        next_tensordict.set((self.group, "sbert_embedding"), self.cached_raw_vector)
    ...
    def _reset(self, tensordict: TensorDictBase, tensordict_reset: TensorDictBase) -> TensorDictBase:
        # 에피소드 초기화 시 상태 리셋
        self.cached_vector = None
        batch_size = tensordict_reset.batch_size[0]
        # 리셋 시 첫 번째 observation에도 벡터를 붙여주기 위해 _step과 동일한 로직 적용
        curr_obs = tensordict_reset.get((self.group, "observation"))
        
        # intention = [0 for _ in range(batch_size)]
        intention = self.get_llm_intention(self.group, tensordict_reset)
        raw_vector, vector = self.intention_to_vector(intention)
        self.cached_raw_vector = raw_vector.detach()
        self.cached_vector = vector.detach() #vector.unsqueeze(1)
        
        new_obs = torch.cat([curr_obs, self.cached_vector], dim=-1)
        tensordict_reset.set((self.group, "observation"), new_obs)
    ...
    def _obs_to_text(self, obs_tensor): # torch.Size([envs_workers, n_agents, obs_dim])
        ...
        for i in range(obs_cpu.shape[0]):
            ...
            for j in range(obs_cpu.shape[1]):
                pos_x, pos_y, vel_x, vel_y = obs_cpu[i, j, 0:4]
            ...
            base_prompt = [
            {
                ...
                "content": [{
                    "type": "text", 
                    "text": "You are a Strategic Commander for a multi-agent system. Your goal is to solve a 'Narrow Corridor Swap' problem. Two agents must swap positions, but the corridor only fits one. You must decide who yields (waits in the side opening) and who proceeds. Your instructions must be spatially logical and promote coordination."
                }]
            },
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": "ENVIRONMENT: A narrow horizontal corridor. Agent 1 starts left, Agent 2 starts right. Their goals are on the opposite ends. There is one asymmetric opening (refuge) in the middle where one agent can wait to let the other pass."},
                    *agent_texts,
                    {"type": "text", "text": "TASK: One agent must enter the middle opening and wait (YIELD), while the other agent passes through the corridor (PROCEED). Simultaneous movement in the narrow part will cause a collision and failure."},
                    {"type": "text", "text": "COMMAND: Based on the current spatial state, determine the optimal immediate action for each agent to safely complete the swap. Consider whether an agent should advance, move into the refuge to clear the path, or exit the refuge if the corridor is now clear. Output your response STRICTLY as a valid JSON list containing exactly two strings (Instruction for Agent 1, Instruction for Agent 2). Do not include any markdown, labels, or extra text. \nExample: [\"The corridor is clear, proceed forward to the goal.\", \"Move into the middle refuge to let the other agent pass.\"]"}
                ]
            }
            ]
    ...
    def get_llm_intention(self, group: str, tensordict: TensorDictBase = None): # -> [batch(120), agent(2)]
        ...
        contents = self.algorithm.pipe(state_description, max_new_tokens=100, batch_size=len(state_description)) # , max_length = None, max_new_tokens=100
        intention = []
        for content in contents:
                ...
                if match:
                    ...
                    try:
                        ...
                        intention.append(["Proceed.", "Yield."])
                else:
                    intention.append(["Proceed.", "Yield."])
        return intention
    ...
    def intention_to_vector(self, intention: list[list[str]]):
        ...
        with torch.no_grad():
            raw_vector = self.algorithm.sbert.encode(flat_intentions,
                                                      convert_to_tensor=True,
                                                      show_progress_bar=False,
                                                      )
        raw_vector = raw_vector.view(batch_size, n_agents, -1)
        vector = self.algorithm.linear(raw_vector)

        # oracle = torch.tensor([[1.0, 0.0], [0.0, 1.0]], device=self.algorithm.pipe.model.device, dtype=torch.float32)
        # vector = oracle.unsqueeze(0).expand(batch_size, -1, -1) # shape: (batch_size, n_agents, intention_vector_size)
        return raw_vector, vector
    
    def transform_observation_spec(self, observation_spec):
        # print(f"Original observation spec: {observation_spec}")
        ...
        if (self.group, "observation") in observation_spec.keys(True):
            ...
            if (self.group, "sbert_embedding") not in observation_spec.keys(True):
                new_shape = torch.Size([*orig_spec.shape[:-1], orig_spec.shape[-1] + self.algorithm.intention_vector_size])
                
                observation_spec[self.group, "observation"] = Unbounded(
                    shape=new_shape, 
                    device=orig_spec.device
                )
        ...
        # 2. State Spec 업데이트 (Critic이 state를 사용하는 경우 대비)
        if (self.group, "state") in observation_spec.keys(True):
            orig_spec = observation_spec[self.group, "state"]
            if (self.group, "sbert_embedding") not in observation_spec.keys(True):
                new_shape = torch.Size([*orig_spec.shape[:-1], orig_spec.shape[-1] + self.algorithm.intention_vector_size])
                
                observation_spec[self.group, "state"] = Unbounded(
                    shape=new_shape, 
                    device=orig_spec.device
                )
                sbert_shape = torch.Size([*orig_spec.shape[:-1], 384])
...
class LLM_Intention(Transform):
    ...
    def _obs_to_text(self, obs_tensor, prev_text=None): # torch.Size([envs_workers, n_agents, obs_dim])
        obs_cpu = obs_tensor.cpu().numpy() # 한 번에 CPU로 내림
    ...
    def get_llm_intention(self, group: str, tensordict: TensorDictBase = None): # -> [batch(120), agent(2)]
        ...
        with torch.no_grad():
            contents = self.algorithm.pipe(state_description, max_new_tokens=200, batch_size= min(32, len(state_description)))#len(state_description)) # , max_length = None, max_new_tokens=100
        print("*" * 50)
        print(f"State description for LLM:\n{state_description[0]}")
        # print()
        print(f"LLM raw output: {contents[0][0]['generated_text'][-1]['content']}")
        intention = []
        for content in contents:
            raw_text = content[0]["generated_text"][-1]["content"].strip()
            match = re.search(r'\[.*?\]', raw_text, re.DOTALL)
    ...
    def _step(self, tensordict: TensorDictBase, next_tensordict: TensorDictBase) -> TensorDictBase:
        ...
        with torch.no_grad():
            current_vector = self.algorithm.linear(final_raw_vector)
            
        new_obs = torch.cat([curr_obs, current_vector], dim=-1)
        next_tensordict.set((self.group, "observation"), new_obs)
    ...
    def _reset(self, tensordict: TensorDictBase, tensordict_reset: TensorDictBase) -> TensorDictBase:
        ...
        self.prev_text = [["None", "None"] for _ in range(curr_obs.shape[0])]
        intention = self.get_llm_intention(self.group, tensordict_reset)
        raw_vector = self.intention_to_vector(intention).detach()
        self.prev_text = intention
        with torch.no_grad():
            current_vector = self.algorithm.linear(raw_vector)
    ...
    def transform_observation_spec(self, observation_spec):
        ...
        if (self.group, "observation") in observation_spec.keys(True):
            if (self.group, "sbert_embedding") not in observation_spec.keys(True):
                orig_spec = observation_spec[self.group, "observation"]
                new_shape = torch.Size([*orig_spec.shape[:-1], orig_spec.shape[-1] + self.algorithm.intention_vector_size])
                observation_spec[self.group, "observation"] = type(orig_spec)(
                    shape=new_shape, device=orig_spec.device
                )
        ...
        if (self.group, "state") in observation_spec.keys(True):
            orig_spec = observation_spec[self.group, "state"]
            new_shape = torch.Size([*orig_spec.shape[:-1], orig_spec.shape[-1] + self.algorithm.intention_vector_size])
            observation_spec[self.group, "state"] = type(orig_spec)(
                shape=new_shape, device=orig_spec.device
            )
